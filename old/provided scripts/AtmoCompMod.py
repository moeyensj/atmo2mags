...
class AtmoComp:
    ...
    def plotAtmosRatio(self, this_seczlist, xlim=(300, 1100),
		...
		# Changed atm_trans to atmo_trans x4
        trans_ratio = numpy.zeros(len(self.wavelen), dtype='float')
        zref = self.seczToString(this_seczlist[0])
        eps = 1e-30
        for comp in self.atmo_comp: 
            tmp = numpy.where(self.atmo_trans[zref]==0, eps, self.atmo_trans[zref])
        pylab.figure()        
        for z in this_seczlist:            
            trans_ratio = self.atmo_trans[self.seczToString(z)][comp] / self.atmo_trans[zref][comp]
